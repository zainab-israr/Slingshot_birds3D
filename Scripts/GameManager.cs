...
public class GameManager : MonoBehaviour
{
    ...
    public Color clr= new Color(78F, 228F, 255F, 107F);
    void Start()
    {
        sound = GetComponents<AudioSource>();
        WoodDestruction = sound[0];
        IceDestruction = sound[1];
        PigDestroy = sound[2];
        BirdDestroy = sound[3];
        PigHit = sound[4];
        LevelCleared = sound[5];
        LevelFailed = sound[6];
        LevelCompleted = sound[7];
        nature = sound[8];
        if (Instance == null)
        {
            Instance = this;
    ...
    void Update()
    {
        ...
        else if(PauseButton.GameIsResumed)
        {
            nature.pitch = 1f;
        }
    }
    ...
    IEnumerator AddFinalScores()
    {
        ...
        foreach (StillBird stillBird in FindObjectsOfType<StillBird>())
        {
            AddScore(10000, stillBird.transform.Find("Bird Body").transform.position, clr);
        }
        foreach (Bird bird in FindObjectsOfType<Bird>())
        {
            AddScore(10000, bird.transform.position, clr);
        }
